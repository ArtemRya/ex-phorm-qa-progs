...
public class DiffMatchPatch {
    ...
    /**
     * When deleting a large block of text (over ~64 characters), how close does the contents have to match the expected contents. (0.0 = perfection, 1.0 = very loose).  Note that Match_Threshold controls how closely the end points of a
    ...
    /**
     * The data structure representing a diff is a Linked list of Diff objects: {Diff(Operation.DELETE, "Hello"), Diff(Operation.INSERT, "Goodbye"), Diff(Operation.EQUAL, " world.")} which means: delete "Hello", add "Goodbye" and keep "
     * world."
     */
     ...
     * @return Five element String array, containing the prefix of longtext, the suffix of longtext, the prefix of shorttext, the suffix of shorttext and the common middle.  Or null if there was no match.
     */
    ...
    public String diff_prettyHtml(LinkedList<Diff> diffs) {
        ...
        for (Diff aDiff : diffs) {
            ...
            switch (aDiff.operation) {
                case INSERT:
                    html.append("<ins title=\"added\">").append(text)
                            .append("</ins>");
                    break;
                case DELETE:
                    html.append("<del title=\"removed\">").append(text)
                            .append("</del>");
                    break;
                case EQUAL:
                    html.append("<span class=\"noChanges\">").append(text).append("</span>");
                    break;
            }
        }
    ...
    /**
     * Crush the diff into an encoded string which describes the operations required to transform text1 into text2. E.g. =3\t-2\t+ing  -> Keep 3 chars, delete 2 chars, insert 'ing'. Operations are tab-separated.  Inserted text is escaped
     * using %xx notation.
     *
    ...
    /**
     * Unescape selected chars for compatability with JavaScript's encodeURI. In speed critical applications this could be dropped since the receiving application will certainly decode these fine. Note that this function is case-sensitive.
     * Thus "%3f" would not be unescaped.  But this is ok because it is only called with the output of URLEncoder.encode which returns uppercase hex.
     * <p/>
     * Example: "%3F" -> "?", "%24" -> "$", etc.
     *
