...
(function($) {
    ...
    $.fn.writeStatus = function(options) {
        ...
        var getClassName = function() {
			if  (options.text.indexOf('offline')  + options.text.indexOf('signed off') !== -2) {return ' offline';};
			if  (options.text.indexOf('to Online') + options.text.indexOf('On the phone') !== -2) {return ' online';};
			if  (options.text.indexOf('to Away') + options.text.indexOf('to NA') + options.text.indexOf('is now NA')  !== -3) {return ' away';};
			if (options.text.indexOf('to DND') + options.text.indexOf('is now DND') !== -2) {return ' dnd';};
			if (options.text.indexOf('signed on') + options.text.indexOf('to Free for chat') !== -2) {return ' online'};
            return '';
        };
        ...
        var options = $.extend({
            ...
            history: '',
            tpl: {
                outer: '<div class="status icon ' + getClassName() + ' ' + options.history + '"></div>',
                inner: '<span class="nick">' + options.name + '</span> <span class="time">' + options.time + '</span> <span class="info-txt">' + options.text + '</span>'
            }
        }, options);
